...
findEven zero = zero , (base , tt)
findEven (suc zero) = suc (suc zero) , (step base , tt)
findEven (suc (suc m)) with findEven m
... | x , (x₁ , x₂) = {!!}
--findEven (suc (suc m)) with findEven m
--... | n , (p , q) = suc (suc n) , ((step (step p)), q)

--  We can extract the algorithm simply by projection because Σ is constructive.

findEven' : Σ[ f ∶ (ℕ → ℕ) ] ((m : ℕ) → (m ≤ f m) × Even (f m))
findEven' = (λ m → proj₁ (findEven m)) , λ m → proj₂ (findEven m)
